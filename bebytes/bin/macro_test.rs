...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // let client_setup_response = ArrayedStruct::new(Modes::new(0), [0; 80], [0; 64], [0; 16]);
    ...
    // let with_sliced = WithSliced {
    //     slice: &[1, 2, 3, 4, 5],
    // };
    // let with_sliced_bytes = with_sliced.to_be_bytes();
    // let slice = with_sliced_bytes.as_ref();
    // println!("WithSliced: {:?}", with_sliced_bytes);
    // {
    //     let re_with_sliced = WithSliced::try_from_be_bytes(slice)?;
    //     println!("ReWithSliced: {:?}", re_with_sliced);
    //     assert_eq!(with_sliced, re_with_sliced.0);
    // }
